...
export const EmptyChatState = memo(
  ({ onQuickAction, onFocusInput }: EmptyChatStateProps) => {
    const { addLog } = useLog();

    const handleContainerClick = useCallback(() => {
      addLog("info", "EmptyChatState container clicked - focusing input");
      onFocusInput();
    }, [onFocusInput, addLog]);

    const handleButtonClick = useCallback(
      (prompt: string) => (e: React.MouseEvent) => {
        addLog("info", "Quick action button clicked", { prompt });
        e.stopPropagation();
        onQuickAction(prompt);
      },
      [onQuickAction, addLog]
    );

    const handleCustomEndpointsClick = useCallback(
      (e: React.MouseEvent) => {
        addLog("info", "Custom endpoints link clicked");
        e.stopPropagation();
        windowsCommands.windowShow({ type: "settings" }).then(() => {
          ...
          addLog("info", "Settings window opened and navigated to AI tab");
        });
      },
      [addLog]
    );
    ...
    return (
      ...
      >
        <div className="flex items-center gap-2 mb-4">
          ...
          <Badge
            variant="secondary"
            className="text-[10px] px-1.5 py-0.5 bg-blue-100 text-blue-800 border-blue-200"
          >
            Beta
          </Badge>
        </div>
